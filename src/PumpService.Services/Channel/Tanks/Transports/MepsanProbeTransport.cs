...
{
    public class MepsanProbeTransport : SerialTransport
    {
        #region Constructor

        public MepsanProbeTransport(IStreamResource streamResource)
            : base(streamResource)
        {
        ...
        {
            MepsanDDAMessage msg = (MepsanDDAMessage)message;
        ...
        private byte[] ReadRequestBlock()
        {
            ...
            do
            {
                ...
                {
                    //log.Warn("MepsanProbeTransport ReadRequestBlock Başarısız Oldu");
                    break;
                }
            } while (!messageRead);
