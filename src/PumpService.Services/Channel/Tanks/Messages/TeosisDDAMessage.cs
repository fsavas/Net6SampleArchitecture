...
{
    public abstract class TeosisDDAMessage
    {
        #region Methods
        ...
        public double GetProductLevel(byte[] data)
        {
            ...
            byte[] productBytes = { bytes[3], bytes[4], bytes[5], bytes[6], bytes[7], bytes[8] };
            ...
            catch (Exception e)
            {
                ...
                //taskService.SaveTarpetTaskLogs("TeosisProbe", "HATALI ÖLÇÜM " + "FuelHeightMm=" + value.ToString(), (int)TarpetLogCodes.ProbeFuelLevelReadError);
            }
        ...
        public double GetInterfaceLevel(byte[] data)
        {
            ...
            byte[] productBytes = { bytes[10], bytes[11], bytes[12], bytes[13], bytes[14], bytes[15] };
            ...
            catch (Exception e)
            {
                ...
                //taskService.SaveTarpetTaskLogs("TeosisProbe", "HATALI ÖLÇÜM " + "WaterHeightMm=" + value.ToString(), (int)TarpetLogCodes.ProbeWaterLevelReadError);
            }
        ...
        public double GetAverageTemperature(byte[] data)
        {
            ...
            byte[] productBytes = { bytes[17], bytes[18], bytes[19], bytes[20] };
            ...
            try
            {
                value = Double.Parse(str.Trim(), CultureInfo.GetCultureInfo("tr-TR"));
            }
            catch (Exception e)
            {
                value = -99;
                ...
                //taskService.SaveTarpetTaskLogs("TeosisProbe", "HATALI ÖLÇÜM " + "Temperature=" + value.ToString(), (int)TarpetLogCodes.ProbeTemperatureReadError);
            }
